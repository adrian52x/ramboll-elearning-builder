...
import { CreateELearningDto, CreateStepDto, BlockType, StepBlockDTO } from "@/types";
...
const BasicInfoTab = ({ data, onUpdate, onCancel }: { data: CreateELearningDto; onUpdate: (updates: Partial<CreateELearningDto>) => void; onCancel: () => void }) => {
...
const StructureTab = ({ data, onUpdate }: { data: CreateELearningDto; onUpdate: (updates: Partial<CreateELearningDto>) => void }) => {
    const addStep = () => {
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Define steps and stepBlocks for your e-learning course. At least one step is required.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
                    ...
                    <div key={stepIndex} className="border border-border rounded-lg p-4 space-y-4">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center">
                                ...
                                <Button type="button" variant="outline" size="sm" onClick={() => addBlockToStep(stepIndex)}>
                                    Add Block
                                </Button>
                            </div>
                            ...
                            {step.stepBlocks.map((block, blockIndex) => (
                                <div key={blockIndex} className="bg-muted p-3 rounded space-y-2">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <Button type="button" variant="destructive" size="sm" onClick={() => removeBlock(stepIndex, blockIndex)}>
                                            Remove
                                        </Button>
                                    </div>
                                    {block.newBlock && (
                                        <>
                                            <Input
                                                ...
                                                onChange={(e) =>
                                                    updateBlock(stepIndex, blockIndex, {
                ...
                {data.steps.length === 0 && <p className="text-muted-foreground text-center py-4">No steps added yet. Add at least one step.</p>}
